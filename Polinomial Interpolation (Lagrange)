...
xplt = np.linspace(x[0],x[-1], 1000) #x0 e x-1 são o primeiro e último valores do array, [número de pontos do plot]
yplt = np.array([], float) #deixar em branco pois conforme ele roda a gente vai enviando os valores do y

for xp in xplt: #agora o xp terá os valores dos 50 pontos dentro do xplt
    yp = 0
...
#plot
plt.plot(x,y,'rs',xplt,yplt,'b') #plot dos pontos, r=red, o=circles, b=blue, -=continuous line
plt.xlabel('x')
plt.ylabel('y')
print(xplt)
print(yplt)
